...
\subsubsection{Kích thước lưu trữ}

Với ba vector nhúng cho mỗi bản ghi và mỗi vector có 768 chiều kiểu float32, kích thước lưu trữ cho mỗi công việc hoặc CV vào khoảng 9KB ($768 \times 4 \times 3 = 9.216$ byte). Tổng dung lượng lưu trữ vector phụ thuộc vào số lượng bản ghi trong hệ thống và có thể được tính theo công thức $9\text{KB} \times (N_{job} + N_{cv})$.
...
Các tham số $K_1$, $K_2$, $K_3$ được cấu hình qua biến môi trường, với giá trị mặc định lần lượt là 1000, 100 và 10. Việc tách riêng các tham số này cho phép điều chỉnh linh hoạt tùy theo quy mô dữ liệu và yêu cầu về độ chính xác.
...
\textbf{Vòng 2 - Lọc theo kinh nghiệm.} Vòng thứ hai so sánh vector kinh nghiệm của CV với vector yêu cầu của $K_1$ công việc từ vòng trước để chọn ra $K_2$ công việc có độ khớp cao nhất. Việc tính toán được thực hiện bằng vòng lặp trực tiếp thay vì xây dựng chỉ mục FAISS mới vì với $K_1$ phần tử, chi phí xây dựng chỉ mục lớn hơn chi phí tính toán trực tiếp. Đối với CV không có kinh nghiệm như sinh viên mới ra trường, điểm mặc định được gán để không loại bỏ hoàn toàn nhưng vẫn ưu tiên các CV có kinh nghiệm phù hợp. Tương tự, công việc không có yêu cầu cụ thể được gán điểm trung tính.

\textbf{Vòng 3 - Lọc theo kỹ năng.} Vòng cuối cùng so sánh vector kỹ năng của CV với vector kỹ năng của $K_2$ công việc từ vòng trước để chọn ra $K_3$ kết quả cuối cùng. Tương tự vòng 2, các trường hợp thiếu thông tin kỹ năng được xử lý bằng điểm mặc định phù hợp.
...
Hệ thống áp dụng nhiều chiến lược tối ưu hóa để giảm thời gian xử lý và tài nguyên tính toán. Hình \ref{fig:optimization-overview} minh họa tổng quan các chiến lược tối ưu hóa được áp dụng.

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    box/.style={rectangle, draw, fill=blue!10, minimum width=4cm, minimum height=1.2cm, text centered, font=\small, rounded corners=3pt},
    arrow/.style={->, >=stealth, thick},
    label/.style={font=\footnotesize, text=gray}
]

% Main optimization strategies
\node[box, fill=yellow!20] (hash) at (-3, 2) {Bộ nhớ đệm\\mã băm nội dung};
\node[box, fill=green!20] (filter) at (3, 2) {Lọc công việc\\đang hoạt động};

% Benefits
\node[box, fill=gray!10] (benefit) at (0, 0) {Giảm thời gian xử lý\\Tiết kiệm tài nguyên};

% Arrows
\draw[arrow] (hash) -- (benefit);
\draw[arrow] (filter) -- (benefit);

% Labels
\node[label] at (-3, 0.8) {Tránh tính lại embedding};
\node[label] at (3, 0.8) {Giảm tập dữ liệu};

\end{tikzpicture}
\caption{Tổng quan các chiến lược tối ưu hóa}
\label{fig:optimization-overview}
\end{figure}
